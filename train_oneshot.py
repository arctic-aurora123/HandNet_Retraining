...
writer = SummaryWriter('runs/0707')
...
def training(train_data, test_data, epoches):
    ...
    for n_epochs in range(epoches):
        ...
        with tqdm(total=epoches) as pbar1:
            ...
            for idx, data in enumerate(train_data):
                ...
                saved = imgs.numpy()
                for i in range(2):
                    temp = np.transpose(saved[i], (1, 2, 0))*(-256)
                    cv2.imwrite(f"images/train_target{i}.jpg", temp)
                imgs = imgs.to(device)
        ...
        if n_epochs % 100 == 0:
            ...
            torch.save(model, f'model_0707_small.pt')
            torch.save(model.state_dict(), 'model_0707_small')    
...
if __name__ == '__main__':
    ...
    train_data = DataLoader(train_dataset, batch_size, shuffle=False, num_workers=1)
    test_dataset = DexYCB(cfg_test)
    test_data = DataLoader(test_dataset, batch_size, shuffle=False, num_workers=1)
    ...
    model = model = torch.load('model_0707_small.pt')
    model = model.cuda()
    ...
    training(train_data, test_data, epochs)
    #training(data, data, epochs)
    writer.flush()
