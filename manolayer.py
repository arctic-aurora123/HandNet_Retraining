...
class ManoLayer(Module):
    ...
    #     batch_size = torch.tensor.shape[0]
    ...
    def __init__(self,
        ...
        hands_mean = hands_mean.copy()[None, ...]
...
#        if self.use_pca or self.joint_rot_mode == 'axisang':
        ...
        #print(th_pose_coeffs.shape)
        th_hand_pose_coeffs = th_pose_coeffs[:, 0 ,self.rot:self.rot + self.ncomps]
        th_full_hand_pose = th_hand_pose_coeffs
        th_hands_mean = self.th_hands_mean.squeeze(1)
        # Concatenate back global rot
        th_full_pose = torch.cat([
            th_pose_coeffs[:, 0, :self.rot], 
        ...
        th_betas = th_betas.squeeze(1)
        #print(th_betas.shape)
